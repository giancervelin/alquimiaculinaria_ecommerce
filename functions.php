...
/**add_action( 'wp_enqueue_scripts', 'alquimia_force_wc_scripts_on_home', 20 );
*function alquimia_force_wc_scripts_on_home() {
...
// Faz a busca do site encontrar Posts e Produtos simultaneamente
function alquimia_busca_global_mista( $query ) {
    if ( $query->is_search() && ! is_admin() && $query->is_main_query() ) {
        $query->set( 'post_type', array( 'post', 'product' ) );
    }
    return $query;
}
add_filter( 'pre_get_posts', 'alquimia_busca_global_mista' );
...
add_action('wp_ajax_alquimia_calcular_frete', 'alquimia_calcular_frete');
add_action('wp_ajax_nopriv_alquimia_calcular_frete', 'alquimia_calcular_frete');

function alquimia_calcular_frete() {
    $product_id = isset($_POST['product_id']) ? absint($_POST['product_id']) : 0;
    $cep        = isset($_POST['cep']) ? wc_clean($_POST['cep']) : '';
    $qty        = isset($_POST['qty']) ? absint($_POST['qty']) : 1;
    $cep        = preg_replace('/[^0-9]/', '', $cep);

    if (!$product_id || strlen($cep) !== 8) {
        wp_send_json_error('Dados insuficientes para o cálculo.');
    }

    // 1. Inicia a sessão se necessário e limpa caches de frete
    if (!WC()->session) {
        wc_load_cart_ajax();
    }
    WC()->shipping()->reset_shipping();

    // 2. Define a localização do cliente globalmente
    WC()->customer->set_shipping_location('BR', '', $cep, '');
    WC()->customer->set_billing_location('BR', '', $cep, '');

    $product = wc_get_product($product_id);
    
    // Verificação de segurança: Peso e Dimensões
    if (!$product->get_weight() && !$product->get_length()) {
        wp_send_json_error('O produto precisa de peso e dimensões para calcular o frete.');
    }

    // 3. Monta o pacote no formato que os plugins (Correios/Melhor Envio) esperam
    $package = array(
        'contents'        => array(
            array(
        ...
        'contents_cost'   => $product->get_price() * $qty,
    ...
    // 4. Calcula o frete disparando os hooks das transportadoras
    $rates = WC()->shipping()->calculate_shipping_for_package($package);
    
    if (empty($rates)) {
        wp_send_json_error('Nenhum método de entrega disponível para este CEP.');
    }

    ob_start();
    echo '<ul class="alquimia-frete-lista">';
    foreach ($rates as $rate) {
        // Fallback: se o label estiver vazio, usa o ID do método
        $label = !empty($rate->label) ? $rate->label : $rate->method_id;
        $cost  = $rate->cost > 0 ? wc_price($rate->cost) : 'Grátis';

        echo '<li class="alquimia-frete-item">';
...
// Shortcode: [alquimia_menu_icons]
add_shortcode('alquimia_menu_icons', function () {
	...
	?>
	<div class="alquimia-menu-actions">
		<div class="alquimia-account">
			<a class="alquimia-account-btn" href="<?php echo esc_url($myaccount_url); ?>">
				<?php echo $logged_in ? esc_html('Olá, ' . $user_name) : esc_html('Entrar / Cadastrar'); ?>
			</a>

			<?php if ($logged_in): ?>
				<div class="alquimia-account-dropdown">
					<a href="<?php echo esc_url($myaccount_url); ?>">Minha conta</a>
					<a href="<?php echo esc_url( trailingslashit($myaccount_url) . 'orders/' ); ?>">Meus pedidos</a>
					<a href="<?php echo esc_url( trailingslashit($myaccount_url) . 'edit-account/' ); ?>">Editar perfil</a>
					<a href="<?php echo esc_url( wc_logout_url( home_url('/') ) ); ?>">Sair</a>
				</div>
			<?php endif; ?>
		</div>

		<button class="alquimia-cart-btn" type="button" aria-label="Abrir carrinho" data-alquimia-cart-toggle>
			...
			<span class="alquimia-cart-count">
				<?php echo esc_html( WC()->cart ? WC()->cart->get_cart_contents_count() : 0 ); ?>
			</span>
		</button>
	</div>

	<!-- Drawer lateral (nativo via woocommerce_mini_cart) -->
	<div class="alquimia-cart-overlay" data-alquimia-cart-overlay></div>

	<aside class="alquimia-cart-drawer" data-alquimia-cart-drawer aria-hidden="true">
		<header class="alquimia-cart-header">
			<strong>Seu carrinho</strong>
			<button class="alquimia-cart-close" type="button" aria-label="Fechar carrinho" data-alquimia-cart-close>×</button>
		</header>

		<div class="alquimia-cart-body">
			<div class="alquimia-mini-cart-wrap">
				<?php woocommerce_mini_cart(); ?>
			</div>
		</div>

		<footer class="alquimia-cart-footer">
			<a class="alquimia-cart-checkout" href="<?php echo esc_url($checkout_url); ?>">Finalizar compra</a>
		</footer>
	</aside>
	<?php
	...
	ob_start();
	?>
	<span class="alquimia-cart-count"><?php echo esc_html( WC()->cart->get_cart_contents_count() ); ?></span>
	<?php
	$fragments['span.alquimia-cart-count'] = ob_get_clean();

	// Atualiza o conteúdo do mini-cart dentro do drawer
	ob_start();
	?>
	<div class="alquimia-mini-cart-wrap"><?php woocommerce_mini_cart(); ?></div>
	<?php
	$fragments['div.alquimia-mini-cart-wrap'] = ob_get_clean();
...
add_action('wp_enqueue_scripts', function () {
	...
	// Script do drawer (inline para você copiar rápido; depois pode virar arquivo)
	...
	(function(){
		...
		function openDrawer(){
			var drawer = qs('[data-alquimia-cart-drawer]');
			var overlay = qs('[data-alquimia-cart-overlay]');
			if(!drawer || !overlay) return;
			drawer.classList.add('is-open');
			overlay.classList.add('is-open');
			drawer.setAttribute('aria-hidden','false');
			document.body.classList.add('alquimia-cart-open');
		}
		...
		function closeDrawer(){
			...
			document.body.classList.remove('alquimia-cart-open');
		}
		...
		document.addEventListener('click', function(e){
			...
			// abrir
			if (t && (t.matches('[data-alquimia-cart-toggle]') || t.closest('[data-alquimia-cart-toggle]'))) {
				e.preventDefault();
				openDrawer();
				return;
			}
			...
			// fechar
			if (t && (t.matches('[data-alquimia-cart-close]') || t.closest('[data-alquimia-cart-close]'))) {
				e.preventDefault();
				closeDrawer();
				return;
			}
			...
			// overlay fecha
			if (t && t.matches('[data-alquimia-cart-overlay]')) {
				closeDrawer();
				return;
			}
		});
		...
		// Quando um produto é adicionado, abre o drawer automaticamente (nativo)
		...
		// CSS do drawer + botões (inline para copiar rápido)
	wp_add_inline_style('woocommerce-general', "
	.alquimia-menu-actions{display:flex;align-items:center;gap:12px}
	.alquimia-account{position:relative}
	.alquimia-account-btn{display:inline-block;padding:10px 16px;background:#E3B93A;color:#fff;border-radius:4px;text-decoration:none;font-size:14px;font-weight:600;white-space:nowrap}
	.alquimia-account-btn:hover{background:#D4A520;color:#fff}
	.alquimia-account-dropdown{display:none;position:absolute;right:0;top:calc(100% + 8px);min-width:200px;background:#E3B93A;border-radius:4px;box-shadow:0 8px 24px rgba(0,0,0,.18);overflow:hidden;z-index:99999}
	.alquimia-account:hover .alquimia-account-dropdown{display:block}
	.alquimia-account-dropdown a{display:block;padding:12px 14px;color:#fff;text-decoration:none;font-size:14px}
	.alquimia-account-dropdown a:hover{background:#D4A520;color:#fff}

	.alquimia-cart-btn{position:relative;display:inline-flex;align-items:center;justify-content:center;width:40px;height:40px;background:#E3B93A;color:#fff;border:0;border-radius:4px;cursor:pointer}
	.alquimia-cart-btn:hover{background:#D4A520}
	.alquimia-cart-count{position:absolute;top:-8px;right:-8px;min-width:22px;height:22px;padding:0 6px;display:flex;align-items:center;justify-content:center;background:#ff4444;color:#fff;border-radius:999px;font-size:12px;font-weight:700;border:2px solid #fff}

	.alquimia-cart-overlay{position:fixed;inset:0;background:rgba(0,0,0,.45);opacity:0;visibility:hidden;transition:.2s;z-index:99998}
	.alquimia-cart-overlay.is-open{opacity:1;visibility:visible}
	.alquimia-cart-drawer{position:fixed;top:0;right:0;height:100vh;width:380px;max-width:92vw;background:#fff;transform:translateX(100%);transition:.25s;z-index:99999;display:flex;flex-direction:column}
	.alquimia-cart-drawer.is-open{transform:translateX(0)}
	.alquimia-cart-header{display:flex;align-items:center;justify-content:space-between;padding:18px;border-bottom:1px solid #eee;background:#f9f9f9}
	.alquimia-cart-close{background:none;border:0;font-size:28px;line-height:1;cursor:pointer;color:#666}
	.alquimia-cart-body{flex:1;overflow:auto;padding:16px}
	.alquimia-cart-footer{padding:16px;border-top:1px solid #eee;background:#f9f9f9}
	.alquimia-cart-checkout{display:block;text-align:center;padding:12px;border-radius:6px;background:#E3B93A;color:#fff;text-decoration:none;font-weight:700}
	.alquimia-cart-checkout:hover{background:#D4A520;color:#fff}

	body.alquimia-cart-open{overflow:hidden}

	/* Mini-cart: pequenos ajustes para ficar “clean” */
	.alquimia-cart-drawer .woocommerce-mini-cart__empty-message{margin:0;color:#666}
	.alquimia-cart-drawer .woocommerce-mini-cart-item{display:flex;gap:10px;padding:12px 0;border-bottom:1px solid #eee}
	.alquimia-cart-drawer .woocommerce-mini-cart-item a{color:#333;text-decoration:none}
	.alquimia-cart-drawer .woocommerce-mini-cart__total{margin-top:12px;font-weight:700}
	.alquimia-cart-drawer .woocommerce-mini-cart__buttons a{display:block;margin-top:10px;text-align:center;padding:10px;border-radius:6px}
	");
});
...
// Handler PHP (backend cart)
add_action('wp_ajax_nopriv_update_mini_cart_qty', 'update_mini_cart_qty_handler');
add_action('wp_ajax_update_mini_cart_qty', 'update_mini_cart_qty_handler');

function update_mini_cart_qty_handler() {
  if (!wp_verify_nonce($_POST['nonce'], 'wc_cart_nonce')) {
    wp_die();
  }
...
add_action('wp_footer', 'custom_cart_enhanced');

function custom_cart_enhanced() { ?>
<script>
jQuery(document).ready(function($) {

  const nonce = '<?php echo wp_create_nonce("wc_cart_nonce"); ?>';
  ...
  function updateHeaderCartCount() {
    ...
    $('.woocommerce-mini-cart-item').each(function() {
      const qty = parseInt($(this).find('.qty-display').text()) || 0;
      count += qty;
    });
    $('.alquimia-cart-count').text(count);
  }

  // Separa imagem em coluna própria e nome em coluna própria
  function splitMiniCartImage() {
    $('.alquimia-mini-cart-wrap .woocommerce-mini-cart-item').each(function() {
      const $item = $(this);

      if (!$item.find('.mini-cart-image-col').length) {
        $item.prepend('<div class="mini-cart-image-col"></div>');
      }

      if (!$item.find('.mini-cart-title-col').length) {
        $item.prepend('<div class="mini-cart-title-col"></div>');
      }

      const $imageCol = $item.find('.mini-cart-image-col').first();
      const $titleCol = $item.find('.mini-cart-title-col').first();

      const $link = $item.find('a[href*="/produto/"]').first();

      if ($link.length) {
        const $img = $link.find('img').first();

        if ($img.length && !$imageCol.children().length) {
          $imageCol.append($img);
        }

        if (!$titleCol.text().trim()) {
          const titleText = $link.clone().children('img').remove().end().text().trim();
          $titleCol.text(titleText);
        }
  ...
  // Subtotal client-side instantâneo
  function updateSubtotal() {
    let total = 0;

    $('.woocommerce-mini-cart-item').each(function() {
      const $item = $(this);
      const qty = parseInt($item.find('.qty-display').text()) || 1;
      const priceMatch = $item.find('.quantity .woocommerce-Price-amount bdi').text().match(/R\$[\s ]*([\d,]+)/);
      const unitPrice = priceMatch ? parseFloat(priceMatch[1].replace(',', '.')) : 0;
      total += qty * unitPrice;
    });

    const totalStr = '<span class="woocommerce-Price-currencySymbol">R$</span>&nbsp;' +
      total.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });

    $('.alquimia-mini-cart-wrap .woocommerce-mini-cart__total bdi, .alquimia-cart-footer .woocommerce-mini-cart__total bdi').html(totalStr);
  }
  ...
  function updateItemTotals() {
    $('.woocommerce-mini-cart-item').each(function() {
      ...
      let $itemPrice = $item.find('.item-total-price');
      if (!$itemPrice.length) {
        $itemPrice = $('<div class="item-total-price"></div>');
        $item.append($itemPrice);
      }

      $itemPrice.text(formatBRL(itemTotal));
    });
  }

  // Footer
  function setupFooter() {
    const footer = $('.alquimia-cart-footer');
    const bodySubtotal = $('.alquimia-mini-cart-wrap .woocommerce-mini-cart__total').first();

    if (bodySubtotal.length) {
      footer.find('.woocommerce-mini-cart__total').remove();
      const clone = bodySubtotal.clone().css('display', 'block');
      footer.prepend(clone);
    }

    if (!footer.find('.add-more-products-btn').length) {
      const addBtn = $('<a href="https://alquimiaculinaria.online/loja/" class="add-more-products-btn">Adicionar mais produtos</a>');
      addBtn.on('click', function(e) {
        e.preventDefault();
        $('[data-alquimia-cart-close]').click();
        setTimeout(() => window.location.href = 'https://alquimiaculinaria.online/loja/', 300);
      });

      footer.find('.alquimia-cart-checkout').before(addBtn);
    }
  ...
  function addQtyButtons() {
    $('.alquimia-mini-cart-wrap .quantity').each(function() {
      if (!$(this).siblings('.qty-selector').length) {
        const $span = $(this);
        const qtyMatch = $span.text().match(/(\d+)/);
        const qty = qtyMatch ? parseInt(qtyMatch[1]) : 1;
        const cartKey = $span.closest('.mini_cart_item').find('.remove').data('cart_item_key');

        const $selector = $(`
          <div class="qty-selector" style="display:flex;align-items:center;gap:5px;margin:5px 0;">
            <button class="qty-btn minus" data-key="${cartKey}" style="width:30px;height:30px;border-radius:50%;background:#666;color:white;border:none;font-weight:bold;cursor:pointer;">−</button>
            <div class="qty-display" style="min-width:30px;text-align:center;font-weight:bold;font-size:16px;">${qty}</div>
            <button class="qty-btn plus" data-key="${cartKey}" style="width:30px;height:30px;border-radius:50%;background:#666;color:white;border:none;font-weight:bold;cursor:pointer;">+</button>
          </div>
        `);

        $span.hide().after($selector);
      }
    });
  }

  // +/- : visual + AJAX backend + subtotal instantâneo
  $(document.body).on('click', '.qty-btn', function(e) {
    e.preventDefault();

    const $btn = $(this);
    const $display = $btn.siblings('.qty-display');
    const cartKey = $btn.data('key');

    let qty = parseInt($display.text()) || 1;
    qty = $btn.hasClass('plus') ? qty + 1 : Math.max(0, qty - 1);
    ...
    $.post('<?php echo admin_url("admin-ajax.php"); ?>', {
      ...
      if (response.fragments) {
        $('.woocommerce-mini-cart.cart_list').replaceWith(
          $(response.fragments['div.widget_shopping_cart_content']).find('.woocommerce-mini-cart.cart_list')
        );

        $('.alquimia-mini-cart-wrap .quantity').show();
        addQtyButtons();
        setupFooter();
        updateSubtotal();
        updateItemTotals();
...
// SMTP Brevo FINAL - AUTH PLAIN forçado
add_action('init', function() {
    add_filter('wp_mail_from', function($email) {
        return 'atendimento@alquimiaculinaria.online';
    });
    add_filter('wp_mail_from_name', function($name) {
        return 'Alquimia Culinária';
    });
});
...
add_action('phpmailer_init', function($phpmailer) {
    ...
    $phpmailer->AuthType   = 'PLAIN';  // ✅ Força PLAIN (resolve 535 error)
    error_log('Brevo SMTP carregado: ' . $phpmailer->Username);

    $phpmailer->Port       = 587;
    $phpmailer->SMTPSecure = 'tls';
    $phpmailer->Username   = 'key removed';
    $phpmailer->Password   = 'key removed';
    $phpmailer->From       = 'atendimento@alquimiaculinaria.online';
    $phpmailer->FromName   = 'Alquimia Culinária';
});
...
add_action('pre_get_posts', 'fix_busca_divi_woo');
function fix_busca_divi_woo($query) {
    if (!is_admin() && $query->is_main_query() && isset($_GET['post_type']) && $_GET['post_type'] === 'post,product') {
        $query->set('post_type', array('post', 'product'));
...
/**
 * Shortcode para Menu Sanduíche Premium com Subitens
 * Uso: [hamburger_menu menu="Principal" cor_principal="#333" cor_secundaria="#D42626"]
 * Suporta hierarquia de menus nativa do WordPress
 */

function hamburger_menu_premium( $atts ) {
    $atts = shortcode_atts( array(
        'menu'              => 'Principal',
        'cor_principal'     => '#333',
        'cor_secundaria'    => '#D42626',
        'cor_subitem'       => '#666',
        'cor_hover'         => '#D42626',
    ), $atts );
    ...
    if ( ! $menu_items ) {
        return '<p style="color: red;">Menu "' . esc_html( $atts['menu'] ) . '" não encontrado.</p>';
    }

    // Organizar itens por hierarquia (parent_id)
    $menu_tree = array();
    $top_items = array();
    ...
    $id_unico = 'hamburger-' . uniqid();

    ob_start();
    ?>

    <div class="hamburger-menu-premium" id="<?php echo esc_attr( $id_unico ); ?>">
        <!-- Botão Hambúrguer -->
        <button 
            type="button" 
            class="hamburger-btn" 
            ...
            aria-expanded="false"
            aria-controls="menu-dropdown-<?php echo esc_attr( $id_unico ); ?>"
            style="--cor-principal: <?php echo esc_attr( $atts['cor_principal'] ); ?>;"
        >
        ...
        <nav 
            ...
            id="menu-dropdown-<?php echo esc_attr( $id_unico ); ?>"
            aria-hidden="true"
            style="
                --cor-principal: <?php echo esc_attr( $atts['cor_principal'] ); ?>;
                --cor-subitem: <?php echo esc_attr( $atts['cor_subitem'] ); ?>;
                --cor-hover: <?php echo esc_attr( $atts['cor_hover'] ); ?>;
            "
        >
            <ul class="hamburger-list">
                <?php foreach ( $menu_tree as $item_id => $item_data ) : ?>
                    <li class="hamburger-item <?php echo ! empty( $item_data['children'] ) ? 'tem-filhos' : ''; ?>">
                        <a 
                            href="<?php echo esc_url( $item_data['item']->url ); ?>" 
                            class="hamburger-link principal"
                        >
                            <span class="link-texto"><?php echo esc_html( $item_data['item']->title ); ?></span>
                            <?php if ( ! empty( $item_data['children'] ) ) : ?>
                                <span class="link-seta">›</span>
                            <?php endif; ?>
                        </a>
                        ...
                        <?php if ( ! empty( $item_data['children'] ) ) : ?>
                            <ul class="hamburger-submenu">
                                <?php foreach ( $item_data['children'] as $subitem ) : ?>
                                    <li class="hamburger-item-sub">
                                        <a 
                                            href="<?php echo esc_url( $subitem->url ); ?>" 
                                            class="hamburger-link secundario"
                                        >
    ...
    .hamburger-line {
        ...
        width: 24px;
        height: 3.5px;
        background-color: #D42626;  /* ← MUDOU PARA VERMELHO */
        border-radius: 2px;
        transition: all 0.35s cubic-bezier(0.645, 0.045, 0.355, 1);
        transform-origin: center;
    }

   /* Estado Aberto - Linha 1 (sobe e rotaciona - CORRIGIDO) */
.hamburger-btn.ativo .line-1 {
    transform: rotate(45deg) translate(6px, 6px);  /* ← VALORES SIMÉTRICOS */
}

/* Estado Aberto - Linha 2 (desaparece) */
.hamburger-btn.ativo .line-2 {
    opacity: 0;
}

/* Estado Aberto - Linha 3 (desce e rotaciona - CORRIGIDO) */
.hamburger-btn.ativo .line-3 {
    transform: rotate(-45deg) translate(6px, -6px);  /* ← VALORES SIMÉTRICOS */
}
    ...
    .hamburger-link.principal {
        ...
        transition: all 0.3s ease;
        background: linear-gradient(to right, transparent 0%, transparent calc(100% - 4px), rgba(0, 0, 0, 0) 100%);
    }

    .hamburger-link.principal:hover,
    .hamburger-item.aberto > .hamburger-link.principal {
        background-color: rgba(212, 38, 38, 0.08);
        color: var(--cor-hover);
        padding-left: 22px;
    }

    .hamburger-link.principal:active {
        background-color: rgba(212, 38, 38, 0.15);
    }
    ...
    .hamburger-item.aberto .link-seta {
    ...
    .hamburger-item.aberto .hamburger-submenu {
        max-height: 500px;
        opacity: 1;
    }
    ...
    /* ========================================
       LINKS DE SUBMENU
       ======================================== */
    .hamburger-link.secundario {
        display: block;
        padding: 11px 18px 11px 28px;
        color: var(--cor-subitem);
        text-decoration: none;
        font-size: 13px;
        font-weight: 500;
    ...
    .hamburger-link.secundario::before {
        content: "•";
        position: absolute;
        left: 16px;
        color: var(--cor-hover);
    ...
    .hamburger-link.secundario:hover {
        background-color: rgba(212, 38, 38, 0.06);
        color: var(--cor-hover);
        padding-left: 32px;
    }

    .hamburger-link.secundario:hover::before {
        opacity: 1;
    }
    ...
    (function() {
        document.addEventListener('DOMContentLoaded', function() {
            const wrapper = document.getElementById('<?php echo esc_js( $id_unico ); ?>');
            if (!wrapper) return;

            const btn = wrapper.querySelector('.hamburger-btn');
            const dropdown = wrapper.querySelector('.hamburger-dropdown');
            const itensComFilhos = wrapper.querySelectorAll('.hamburger-item.tem-filhos');
            ...
            btn.addEventListener('click', function(e) {
                e.stopPropagation();
                const estaAberto = this.classList.contains('ativo');
            ...
            // Toggle subitens
            itensComFilhos.forEach(function(item) {
                const link = item.querySelector('.hamburger-link.principal');
            ...
            dropdown.querySelectorAll('a').forEach(function(link) {
                link.addEventListener('click', function(e) {
                    // Não fechar se for um item com filhos (é um toggle)
                    if (!link.closest('.tem-filhos')) {
                        fecharTudo();
                    }
                });
            });
...
add_shortcode( 'hamburger_menu', 'hamburger_menu_premium' );
...
function change_add_to_cart_text_single( $text, $product ) {
    if ( ! $product->is_in_stock() ) {
        $text = 'FORA DE ESTOQUE';
    }
    return $text;
}
add_filter( 'woocommerce_product_single_add_to_cart_text', 'change_add_to_cart_text_single', 10, 2 );

function change_add_to_cart_text_loop( $text, $product ) {
    if ( ! $product->is_in_stock() ) {
        $text = 'FORA DE ESTOQUE';
    }
    return $text;
}
add_filter( 'woocommerce_product_add_to_cart_text', 'change_add_to_cart_text_loop', 10, 2 );
